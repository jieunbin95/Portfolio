...
const Coffee = ({setOpenModal}) => {
  ...
  return (
    <Container ref={ref}>
     ...
     Swiper.js 라이브러리를 이용해 슬라이드를 만들어보았습니다.
또한, GSAP 애니메이션 라이브러리와 영상을 추가하여 동적인 효과를 주었습니다.
        </p>
        <div className='detail_skill'>
          <FontAwesomeIcon icon={faCheck}/>
          Use Skill
        </div>
        <p className='skill'> HTML  <span style={{margin:'0 30px 0'}}>CSS</span> Javascript <span style={{margin:'0 30px 0'}}>Swiper</span></p>
        <div><FontAwesomeIcon icon={faCheck} /><a href='https://curious-stardust-b804d0.netlify.app/'>Demo-Site</a></div>
        <div><FontAwesomeIcon icon={faCheck}/><a href='https://github.com/jieunbin95/starbucks'>Source Code</a></div>
        </Col>  
      </Row>
    </Container>
  )
